...
//      canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);  
        ...
        for (int i = 0; i < 35; i++) {  
            ...
            int colorJu = surface.textColorJu;
            int colorFen = surface.textColorFen;
            if (isLastMonth(i)) {  
                color = surface.borderColor2;  
            } else if (isNextMonth(i)) {  
            		color = surface.borderColor2;
            }  
           
            if(date[i]>=1&&date[i]<=6||date[i]>=17&&date[i]<=25){
            	drawCellText(canvas, i, date[i] + "", color); 
            }
            else if(date[i]>=7&&date[i]<=16){
            	drawCellText(canvas, i, date[i] + "", colorJu);
            }else if (todayIndex != -1 && i == todayIndex) {  
                color = surface.todayNumberColor;  
            }
            else{
            	 drawCellText(canvas, i, date[i] + "", colorFen);
            }
...
//              }  
    ...
    private class Surface {  
        ...
        private int textColor =Color.parseColor("#53bb00"); 
        private int textColorJu =Color.parseColor("#ff6c54");
        private int textColorFen =Color.parseColor("#f3afcb");
        //private int textColorUnimportant = Color.parseColor("#666666");  
        private int btnColor = Color.parseColor("#666666"); 
        private int borderColor2 = Color.parseColor("#ffffff");  
        ...
        public int cellSelectedColor = Color.parseColor("#FF8484");  
        ...
        public void init() {  
           ...
           // weekHeight = (float) ((temp + temp * 0.3f) * 0.7); //星期高度 
            ...
            float cellTextSize = cellHeight * 0.25f;  
